    ...
    public class FirstAttackAction : ActionBase
    {
        ...
        public override void SetActionQueue(ActionQueueComponent actionQueue)
        {
            ...
            if (actionQueue != null && actionQueue.GetHost() is CharacterBase character)
            {
                ...
                if (turnActionCounterComponent != null)
                {
                    ...
                    UpdateDamage(); // 重新计算伤害
                    Debug.Log("FirstAttackAction: 成功连接到TurnActionCounterComponent");
                }
                else
                {
                    Debug.LogWarning("FirstAttackAction: 未找到TurnActionCounterComponent");
                }
            }
        }
        ...
        private void UpdateDamage()
        {
            ...
            if (turnActionCounterComponent != null)
            {
                ...
                totalDamage = isFirstAction ? BaseDamage + BonusDamage : BaseDamage;

                Debug.Log($"FirstAttackAction: 更新伤害 - 是否首个行动: {isFirstAction}, 伤害: {totalDamage}");
            }
            else
            {
                // 如果没有TurnActionCounterComponent，默认不给予额外伤害
                Debug.LogWarning("FirstAttackAction: 没有TurnActionCounterComponent，使用基础伤害");
            }
